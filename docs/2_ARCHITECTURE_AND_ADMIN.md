...
*   **Acompanhamento de E-commerce & Analytics (GTM & Pinterest Tag):**
    *   Container GTM ID: `GTM-NLQ2QXN6` e Pinterest Tag ID: `2613400859149` (e-mail `encomendas@mbravobycarolina.com`).
    *   Eventos de e-commerce sincronizados e estruturados via `window.dataLayer` e `window.pintrk` (`pagevisit`, `begin_checkout`, `purchase` / `checkout`) dissimilados no `App.tsx` e `AtelierCartDrawer.tsx`.
...
*   **Receita de Acessórios & Embalamento (BOM - Bill of Materials)**:
    *   Na ficha de edição do produto no CMS do Admin e no formulário de Venda Manual, a Carolina pode selecionar e configurar os acessórios e consumíveis da peça: **Etiqueta Couro**, **Caixa Premium**, **Saco Envelope Personalizado M★BRAVO**, **Fecho Correr**, **Forro Algodão (com consumo em metros)** e **Botão Madeira**.
    *   Cada consumível em unidades dispõe de quantidade editável e o forro de algodão dispõe de metragem fracionada configurável (ex: `0.25` m).
    *   O motor backend de inventário em `server.ts` (`calculateOrderMaterials` / `getMaterialsNeededForProduct`) lê atomaticamente esta receita no momento em que a encomenda é confirmada/paga (no site ou no Admin) e abate instantaneamente do `inventory.json` todos os itens configurados multiplicados pela quantidade da encomenda.
...
*   **Sistema de E-mails Transacionais Bilíngues (PT / EN) & Automação Inteligente (`emailService.ts`, `server.ts`, `AtelierCartDrawer.tsx` & `AdminDashboardModal.tsx`):**
    *   **Automação no Checkout:** O sistema captura o idioma ativo do cliente (`locale: 'pt' | 'en'`). Caso a morada de envio seja internacional (país diferente de Portugal ou zona de envio insular/europeia/internacional) ou o checkout tenha sido efetuado em Inglês (`lang === 'en'`), a encomenda assume por defeito o idioma Inglês para todos os e-mails associados à venda.
    *   **Inserção Manual no Painel Admin:** O formulário de criação manual de encomendas (`showManualForm`) e a edição de encomendas (`editingOrderModal`) contam com o seletor dedicado **"Idioma dos E-mails"** `[ Português (PT) | English (EN) ]`, permitindo definir e alterar livremente o idioma de comunicação para cada cliente.
    *   **Templates de Luxo Localizados:** Todos os modelos de e-mail (`generateCustomerEmailHtml`, `generateShippedEmailHtml`, `generateMultibancoEmailHtml`, `generateAdminEmailHtml`) adaptam os seus títulos, saudações, discriminativo de encomenda, prazos de confecção artesanal, notas de atelier e links de rastreamento CTT dinamicamente para Português ou Inglês, preservando a identidade visual de alta-costura e a "Gold Rule" de segurança de pagamentos (`order.status === 'paid'`).
