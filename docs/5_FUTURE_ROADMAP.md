...
*   [x] **Sincronização Completa de Checkouts, UI Editorial Mobile Bottom Sheet ("Slide-up Sheet 92dvh"), Brand Compliance (Apple Pay/Google Pay) & Deteção Dinâmica de Ecossistema:** Unificação do componente e fluxo oficial de checkout em `CartCheckoutModal.tsx`, eliminação de duplicidades no seletor de pagamento (manutenção do motor estável MB WAY, Multibanco e Cartão), reformulação visual em estrutura Bottom Sheet para mobile/tablet (92% de altura, cantos superiores arredondados 28px, header e footer fixos e scroll nativo central com `data-lenis-prevent`), integração de botões expressos 1-clique com marcas vetoriais oficiais em conformidade com as Brand Guidelines, deteção dinâmica de dispositivo/navegador (`isApple` vs `isAndroid`), isolamento estrito de eventos de clique e adaptabilidade fluida a orientação horizontal (`landscape:h-[95dvh]`).
