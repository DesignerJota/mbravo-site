...
*   **Ações Realizadas:**
    ...
    26. **Otimizações de Ergonomia Responsiva & Full-Screen Mobile (`AdminDashboardModal.tsx`):**
    ...
    29. **Verificação Global de Lógica de Cores no Catálogo & Mapeamento Fiel com o Servidor (`App.tsx` & `translations.ts`):**
        *   **Omissão Absoluta em "Cor Padrão" (`colorType === 'fixed'`)**: Nos produtos configurados no Admin como "Cor Padrão", o painel de seleção de cores é 100% omitido no Frontend público (sem swatches, sem rótulos e sem áreas em branco). A indicação de cor é igualmente suprimida na linha de configuração do resumo de pedido, carrinho de compras e e-mails de confirmação.
