...
*   **Ações Realizadas:**
    ...
    35. **Hotfix Layout Flexbox Interno dos Botões de Acessórios (`AdminDashboardModal.tsx`):**
        *   **Empilhamento Vertical (flex-col)**: Reformulada a estrutura interna dos cards de Acessórios & Embalamento nos formulários de Venda Manual e Edição de Produto (CMS) para um layout estritamente vertical (`flex flex-col justify-between h-full min-w-0`).
        *   **Separação de Linhas Sem Overflow**: A Linha 1 agrupa a checkbox e o nome do consumível (`min-w-0 truncate leading-tight`). A Linha 2 aloja horizontalmente o rótulo de quantidade/consumo e o respetivo seletor numérico (`w-14 sm:w-16`), separado por uma divisória sutil. Isto previne qualquer quebra ou transbordamento visual em modais de largura reduzida ou grelhas de 3 colunas em Desktop.
