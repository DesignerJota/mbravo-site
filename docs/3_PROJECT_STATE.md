...
*   **Ações Realizadas:**
    ...
    36. **Integração de Rastreio (GA4, GTM, Clarity) & Proteção Anti-Indexação de API (`index.html` & `server.ts`):**
        *   **Google Analytics 4 & Google Tag Manager**: Integrados os scripts oficiais do GA4 (`G-E3ZXEK6RGR`) e GTM (`GTM-TBNWW6L2` com container no `<head>` e iframe `<noscript>` no `<body>`) diretamente no `index.html`.
        *   **Microsoft Clarity (Heatmaps & Replay)**: Integrado o snippet oficial do Microsoft Clarity (`xuul4vcodg`) para gravação de sessões e mapas de calor de utilizadores.
