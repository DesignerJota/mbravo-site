...
const ProductCardComponent: React.FC<ProductCardProps> = ({ product: rawProduct, i, isFocused, isSubdued, onFocus, onPrevProduct, onNextProduct }) => {
    ...
    const colorType: 'single' | 'bicolor' | 'fixed' = 
        (rawColorType === 'fixed' || rawColorType === 'padrao' || rawColorType === 'padrão')
        ? 'fixed'
        : (rawColorType === 'bicolor'
            ? 'bicolor'
            : (rawColorType === 'single'
                ? 'single'
    ...
    const defaultColorsList = [
        ...
        'DROPS Safran - 08 Creme',
        'DROPS Safran - 12 Bege Claro',
        'DROPS Safran - 68 Cacau',
        'DROPS Safran - 11 Branco'
    ];

    const availableColorOptions = rawColorsList.length > 0 ? rawColorsList : defaultColorsList;
    const defaultPrim = availableColorOptions[0] || 'Natural';
    const defaultDet = availableColorOptions[1] || defaultPrim;

    const initialColor = colorType === 'bicolor' 
        ? `${formatColorName(defaultPrim)} & ${formatColorName(defaultDet)}` 
        : (colorType === 'fixed' ? '' : defaultPrim);
  ...
  paymentRequest.on('shippingaddresschange', (ev) => {
    ...
    const selectedColor = colorType === 'fixed'
        ? ''
        : (colorType === 'bicolor'
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2 justify-center mb-2">
                                    ...
                                    <div className="flex items-center gap-1 text-[#C5A059] text-[8px] sm:text-[8.5px] uppercase tracking-[0.18em] font-mono font-medium">
                                        ...
                                        {currentReview?.id !== 'google-invite' && (
                                            <svg className="w-2.5 h-2.5 fill-current shrink-0 text-[#C5A059]" viewBox="0 0 24 24">
                                                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
                                            </svg>
                                        )}
                                        <span>
                                            {currentReview?.id === 'google-invite' ? 'GOOGLE REVIEWS' : 'GOOGLE VERIFIED REVIEW'}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="mt-2.5 flex flex-col items-center justify-center">
                                    ...
                                    {currentReview?.product ? (
                                        ...
                                        <span className="text-[7.5px] uppercase tracking-[0.15em] text-[#C5A059] flex items-center gap-1 font-mono mt-0.5">
