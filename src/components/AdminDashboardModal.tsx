...
import { X, Package, User, ShoppingBag, Truck, Save, Plus, Search, RefreshCw, Key, Trash2, Eye, Bell } from 'lucide-react';
...
export const AdminDashboardModal: React.FC<AdminDashboardModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleDeleteOrder = async (orderId: string) => {
    if (!confirm(`Tem a certeza que deseja eliminar permanentemente a encomenda ${orderId}?`)) {
      return;
    }
    try {
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex justify-between items-start border-b border-[#243119]/10 pb-4">
                    ...
                    <div className="text-right flex flex-col items-end gap-2">
                      ...
                      >
                        <Trash2 className="w-3.5 h-3.5" /> Eliminar Encomenda
                      </button>
                    </div>
                  </div>
